...
int shortest;
	 ...
	 * http://apcentral
	 * .collegeboard.com/apc/public/repository/ap08_comp_sci_a_frq.pdf
	 * 
	 * <code>
	 * 
	...
	public void testTrip() throws Exception {
		...
		Flight testFlight = new Flight(new Time("06/09/1978 09:00"), new Time(
				"06/09/1978 09:02"));
		testTrip.add(testFlight);
		assertEquals(1, testTrip.getNumberOfFlights());

		Flight testFlight2 = new Flight(new Time("06/09/1978 09:00"), new Time(
				"06/09/1978 09:02"));
	 ...
	 */
	 ...
	 flight1 = new Flight(new Time("06/09/1978 18:00"), new
	 Time("06/09/1978 19:30"));
	 flight2 = new Flight(new Time("06/09/1978 19:30"), new
	 ...
	 * between the arrival of a flight and the departure of the flight
	 * immediately after it, if there are two or more flights in the trip; -1,
	 * if there are fewer than two flights in the trip. The departure time for
	 * each flight is always later than the arrival time of its preceding
	 * flight.
	 * 
	 * 1. Write your answer on paper.
	 * 
...
class Time {
	...
	public int minutesUntil(Time other) {
		long differenceInMilliseconds = this.date.getTime()
				- other.date.getTime();
...
class Trip {
	...
	public void clear() {
	 ...
	 int getShortestLayover(){
		 int shortt = flights.get(0).getArrivalTime().minutesUntil(flights.get(1).getDepartureTime());
		for (int i = 0; i < flights.size()-2; i++) {
			Time s = flights.get(i).getArrivalTime();
			Time e = flights.get(i+1).getDepartureTime();
			int dur = s.minutesUntil(e);
			if (dur<shortt) {
				shortt = dur;
			}
		}
